...
func defaultTestModel() Model {
	return GenerateModel(appPromptChar, shellPromptChar, true, defaultTestMaxHeight, defaultTestWidth)
}
...
func TestModel_HandleUpdate(t *testing.T) {
	...
	t.Run("Pressing confirm on empty input", func(t *testing.T) {
		actualTest := func(closeOnSuccess bool, openAfterEnter bool) {
			m := GenerateModel(appPromptChar, shellPromptChar, closeOnSuccess, defaultTestMaxHeight, defaultTestWidth)
			m.Open(true)
	...
	t.Run("Switching between shell and app mode", func(t *testing.T) {
...
func TestModel_HandleResults(t *testing.T) {
	...
	t.Run("Verify prompt results update", func(t *testing.T) {
		m := GenerateModel(appPromptChar, shellPromptChar, false, defaultTestMaxHeight, defaultTestWidth)
		m.Open(true)
		msg := "Test message"
		m.HandlePromptActionResults(true, msg)
		...
		// Validate close happens when closeOnSuccess is true
		m.HandlePromptActionResults(true, "")
		assert.False(t, m.IsOpen())
	})
}
...
func TestModel_Render(t *testing.T) {
	// Test
	// 1 - Default view with shell mode and gfm prompt mode
	// 2 - User input
	...
	t.Run("Basic Render Checks", func(t *testing.T) {
		m := GenerateModel(appPromptChar, shellPromptChar, true, 10, 40)
		m.setShellMode(true)
		res := ansi.Strip(m.Render())
		assert.Contains(t, res, "t4gfm Prompt (Shell Mode)")
		assert.Contains(t, res, "Get into t4gfm mode")
		m.setShellMode(false)
		res = ansi.Strip(m.Render())
		assert.Contains(t, res, "t4gfm Prompt (t4gfm Mode)")
		assert.Contains(t, res, "Get into Shell mode")
	})
	...
	t.Run("Test User Input", func(t *testing.T) {
		execute := func(input string, expected string) {
			...
			width := 10
			m := GenerateModel(appPromptChar, shellPromptChar, true, 10, width)
			m.Open(true)
	...
	t.Run("Result Message", func(t *testing.T) {
		m := GenerateModel(appPromptChar, shellPromptChar, true, 10, 50)
		m.setShellMode(true)
		m.HandleShellCommandResults(0, "")
		res := ansi.Strip(m.Render())
		assert.Contains(t, res, "t4gfm Prompt (Shell Mode)")
		assert.Contains(t, res, "Success : Command exited with status 0")
		m.HandleShellCommandResults(1, "")
		res = ansi.Strip(m.Render())
		assert.Contains(t, res, "Error : Command exited with status 1")
	})
