...
const (
	testEnvVar1 = "T4GFM_TEST_ENV_VAR1"
	testEnvVar2 = "T4GFM_TEST_ENV_VAR2"
	testEnvVar3 = "T4GFM_TEST_ENV_VAR3"
	testEnvVar4 = "T4GFM_TEST_ENV_VAR4"
)
...
func Test_resolveShellSubstitution(t *testing.T) {
	...
	testdata := []struct {
		...
		},
		{
			...
			command:         fmt.Sprintf("$(echo $(echo hi)) ${%s}", testEnvVar2),
			expectedResult:  fmt.Sprintf("hi%s %s", newLineSuffix, testEnvValues[testEnvVar2]),
			isErrorExpected: false,
			errorToMatch:    nil,
		},
		{
			name:            "Non Existing env var",
			command:         fmt.Sprintf("${%s}", testEnvVar4),
			expectedResult:  "",
			isErrorExpected: true,
			errorToMatch:    envVarNotFoundError{varName: testEnvVar4},
		},
		{
