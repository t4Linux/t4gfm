...
func DefaultModel(maxHeight int, width int) Model {
	return GenerateModel(common.Hotkeys.OpenT4gfmPrompt[0],
		common.Hotkeys.OpenCommandLine[0], common.Config.ShellCloseOnSuccess, maxHeight, width)
}

func GenerateModel(appPromptHotkey string, shellPromptHotkey string, closeOnSuccess bool,
	...
	m := Model{
		...
		appPromptHotkey:   appPromptHotkey,
...
func (m *Model) handleNormalKeyInput(msg tea.KeyMsg) tea.Cmd {
	var cmd tea.Cmd
	switch {
	case m.textInput.Value() == "" && msg.String() == m.appPromptHotkey:
...
func (m *Model) HandlePromptActionResults(success bool, msg string) {
...
func (m *Model) Render() string {
	...
	if !m.shellMode {
		...
		r.AddSection()
		r.AddLines(" '" + m.appPromptHotkey + "' - Get into t4gfm mode")
	}
