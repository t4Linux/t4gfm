...
func GetMetadata(filePath string, metadataFocused bool, et *exiftool.Exiftool) Metadata {
	...
	moveOpenWithToEnd(&meta)
	return meta
}

func moveOpenWithToEnd(meta *Metadata) {
	for i := range meta.data {
		if meta.data[i][0] == keyOpenWith {
			openWith := meta.data[i]
			meta.data = append(meta.data[:i], meta.data[i+1:]...)
			meta.data = append(meta.data, openWith)
			return
		}
	}
}
...
func getSymLinkMetaData(filePath string) Metadata {
	...
	if symlinkErr != nil {
		...
		path := [2]string{keyPath, toTildePath(linkPath)}
		res.data = append(res.data, path)
	}
	res.data = append(res.data, [2]string{keyOpenWith, common.ResolveEnterOpenProgram(filePath)})
	return res
}
...
func getMetaDataUnsorted(filePath string, metadataFocused bool, et *exiftool.Exiftool) Metadata {
	...
	openWith := common.ResolveEnterOpenProgram(filePath)
	if fileInfo.IsDir() {
		openWith = "Enter directory"
	}
	res.data = append(res.data, [2]string{keyOpenWith, openWith})
