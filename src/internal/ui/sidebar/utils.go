...
func (s *Model) HasSearchQuery() bool {
	return s.searchBar.Value() != ""
}
...
func (s *Model) jumpToSectionAfterDivider(divider directory) bool {
	for i := range s.directories {
		if s.directories[i] != divider {
			continue
		}
		for j := i + 1; j < len(s.directories); j++ {
			if s.directories[j].isDivider() {
				break
			}
			s.cursor = j
			s.updateRenderIndex()
...
func (s *Model) JumpToPinned() bool {
	return s.jumpToSectionAfterDivider(pinnedDividerDir)
}

func (s *Model) JumpToDisks() bool {
	return s.jumpToSectionAfterDivider(diskDividerDir)
}

func (s *Model) JumpToList() bool {
	for i := range s.directories {
		if s.directories[i].isDivider() {
			continue
		}
