...
func TestJumpToPinnedAndDisks(t *testing.T) {
	t.Run("jump to pinned", func(t *testing.T) {
		s := defaultTestModel(0, 0, 20, 2, 3, 2)
		ok := s.JumpToPinned()
		assert.True(t, ok)
		assert.Equal(t, pinnedDividerDir, s.directories[s.cursor-1])
	})

	t.Run("jump to list", func(t *testing.T) {
		s := defaultTestModel(5, 0, 20, 2, 3, 2)
		ok := s.JumpToList()
		assert.True(t, ok)
		assert.False(t, s.directories[s.cursor].isDivider())
	})

	t.Run("jump to disks", func(t *testing.T) {
		s := defaultTestModel(0, 0, 20, 2, 3, 2)
		ok := s.JumpToDisks()
		assert.True(t, ok)
		assert.Equal(t, diskDividerDir, s.directories[s.cursor-1])
	})

	t.Run("missing pinned section", func(t *testing.T) {
		s := testModel(0, 0, 20, []string{utils.SidebarSectionHome, utils.SidebarSectionDisks},
			formDirctorySlice(dirSlice(2), dirSlice(0), dirSlice(2), []string{utils.SidebarSectionHome, utils.SidebarSectionDisks}))
		ok := s.JumpToPinned()
		assert.False(t, ok)
	})
}
