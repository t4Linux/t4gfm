...
func (m *Model) UpdateElementsIfNeeded(force bool, displayDotFile bool) {
	...
	if force || !m.shouldSkipPanelUpdate(nowTime) {
		...
		m.refreshDirEntryCountCache()
		m.updateGitMarks(nowTime)
...
func (m *Model) refreshDirEntryCountCache() {
	if len(m.element) == 0 {
		m.dirEntryCount = make(map[string]string)
		return
	}

	counts := make(map[string]string)
	for _, elem := range m.element {
		if !elem.Info.IsDir() {
			continue
		}
		if entries, err := os.ReadDir(elem.Location); err == nil {
			counts[elem.Location] = fmt.Sprintf("%d", len(entries))
			continue
		}
		counts[elem.Location] = "-"
	}
	m.dirEntryCount = counts
}
