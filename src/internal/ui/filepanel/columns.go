...
func (m *Model) gitMark(name string) (string, lipgloss.Color, bool) {
	if !m.gitAware {
...
func (m *Model) renderGitMarkSuffix(name string) string {
	mark, color, ok := m.gitMark(name)
	if !ok {
		return ""
	}
	raw := " " + mark
	return common.StringColorRender(color, common.FilePanelBGColor).Background(common.FilePanelBGColor).Render(raw)
}
...
func (m *Model) renderDelimiter(indexElement int, columnWidth int) string {
	loc := m.GetElementAtIdx(indexElement).Location
	isSelected := m.CheckSelected(loc)
	isCursorRow := indexElement == m.GetCursor() && !m.SearchBar.Focused()
...
func (m *Model) renderFileSize(indexElement int, columnWidth int) string {
	...
	sizeValue += m.renderGitMarkSuffix(elem.Name)
	return common.FilePanelItemRender(
		sizeValue,
		columnWidth,
		isSelected,
