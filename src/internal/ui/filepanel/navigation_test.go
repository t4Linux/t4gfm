...
func Test_filePanelUpDown(t *testing.T) {
	testdata := []struct {
		...
		{
			name:           "Down movement at bottom stays at bottom",
		...
		{
			name:           "Up movement at top stays at top",
			panel:          testModelWithElemCount(0, 0, 12, 10),
			listDown:       false,
			expectedCursor: 0,
			expectedRender: 0,
...
func TestPgUpDown(t *testing.T) {
	testdata := []struct {
		...
		{
			name:           "Page down near end clamps to end",
		...
		{
			name:           "Page up near beginning clamps to start",
		...
		{
			name:           "Page navigation with small element count clamps to end",
...
func TestItemSelectUpDown(t *testing.T) {
	testdata := []struct {
		...
		{
			name: "Selection at boundary clamps at end",
...
func TestVisualSelectUpDown(t *testing.T) {
	...
	assert.True(t, panel.IsVisualSelectMode())
	assert.Equal(t, map[string]int{"/tmp/file2.txt": 1}, panel.selected)

	panel.ItemSelectDown()
	assert.Equal(t, 2, panel.GetCursor())
