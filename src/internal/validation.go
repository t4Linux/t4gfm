...
func (m *model) validateComponentRender() error {
	...
	if m.toggleFooter && !m.isCompactFooterActive() {
		if err := validateRender(
			...
			m.systemPanel.GetWidth(),
			true,
		); err != nil {
			return fmt.Errorf("system panel render validation failed: %w", err)
		}
		if err := validateRender(
		...
		if err := validateRender(
			m.gitPanel.Render(false),
			m.gitPanel.GetHeight(),
			m.gitPanel.GetWidth(),
			true,
		); err != nil {
			return fmt.Errorf("git panel render validation failed: %w", err)
		}
	}
...
func (m *model) validateFinalRender() error { //nolint:gocognit // cumilation of validations
	...
	if m.toggleFooter && !m.isCompactFooterActive() {
		systemPanelPos := compPosition{
			...
			endCol: m.systemPanel.GetWidth() - 1,
		}
		if err := m.validateComponentPlacement(lines, systemPanelPos, true); err != nil {
			return fmt.Errorf("system panel position validation failed: %w", err)
		}
		metadataPos := compPosition{
			stRow:  m.mainPanelHeight + common.BorderPadding,
			stCol:  m.systemPanel.GetWidth(),
			endRow: m.fullHeight - 1,
			endCol: m.systemPanel.GetWidth() + m.fileMetaData.GetWidth() - 1,
		}
		...
		gitPanelPos := compPosition{
			stRow:  m.mainPanelHeight + common.BorderPadding,
			stCol:  m.systemPanel.GetWidth() + m.fileMetaData.GetWidth(),
			endRow: m.fullHeight - 1,
			endCol: m.fullWidth - 1,
		}
		if err := m.validateComponentPlacement(lines, gitPanelPos, true); err != nil {
			return fmt.Errorf("git panel position validation failed: %w", err)
		}
	}
...
func (m *model) IsOverlayModelOpen() bool {
	return m.zoxideModal.IsOpen() || m.helpMenu.IsOpen() || m.promptModal.IsOpen() ||
		...
		m.notifyModel.IsOpen() || m.rangerPrefix != ""
}
