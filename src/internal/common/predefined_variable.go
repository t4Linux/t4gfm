...
func LoadPrerenderedVariables() {
	SideBarSuperfileTitle = SidebarTitleStyle.Render(" " + icon.SuperfileIcon + icon.Space + "t4Linux")
	SideBarHomeDivider = buildSidebarDivider(icon.Home + icon.Space + "Parent")
	SideBarPinnedDivider = buildSidebarDivider(icon.Pinned + icon.Space + underlineFirstLetter("Pinned"))
	SideBarDisksDivider = buildSidebarDivider(icon.Disk + icon.Space + underlineFirstLetter("Disks"))
...
func buildSidebarDivider(title string) string {
	titlePart := SidebarTitleStyle.Render(title)
	remaining := Config.SidebarWidth - lipgloss.Width(title) - 1
	if remaining < 1 {
		remaining = 1
	}
	line := " " + strings.Repeat("─", remaining)
	return titlePart + SidebarDividerStyle.Render(line)
}

func underlineFirstLetter(label string) string {
	if label == "" {
		return ""
	}
	first := SidebarTitleStyle.Copy().Underline(true).Render(label[:1])
	rest := SidebarTitleStyle.Render(label[1:])
	return first + rest
}
