...
func FilePanelItemRenderWithIcon(
	...
	iconData := style.Icon + " "
	iconStyle := StringColorRender(lipgloss.Color(style.Color), bgColor).Background(bgColor)
	if isLink {
		iconData = style.Icon + " -> "
		iconStyle = FilePanelSymlinkStyle.Background(bgColor)
	}
	...
	return iconStyle.Render(iconData) +
		FilePanelItemRender(name, filenameWidth, isSelected, isCursor, bgColor, lipgloss.Left)
}
func FilePanelItemRender(data string,
	...
	if isSelected {
		...
		style = FilePanelItemCursorStyle
	}
