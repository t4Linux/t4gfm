	...
	if err != nil {
		...
		if errors.As(err, &loadError) && loadError != nil {
			if loadError.MissingFields() && !variable.FixConfigFile {
				...
				userMsg += "\nTo add missing fields to configuration file automatically run gfm " +
					"with the --fix-config-file flag `gfm --fix-config-file`"
			}
...
func ValidateConfig(c *ConfigType) error {
	...
	if c.SidebarWidth != 0 && (c.SidebarWidth < 5 || c.SidebarWidth > 25) {
		return errors.New(LoadConfigError("sidebar_width", "Sidebar width must be 5–25, or 0 to hide the sidebar."))
	}
	...
	if err != nil {
		...
		if errors.As(err, &loadError) {
			if loadError.MissingFields() && !variable.FixHotkeys {
				// Had missing fields and we did not fix
				userMsg += "\nTo add missing fields to hotkeys file automatically run gfm " +
					"with the --fix-hotkeys flag `gfm --fix-hotkeys`"
			}
			toExit = loadError.IsFatal()
		}
		if toExit {
...
// LoadAllDefaultConfig : Load all default configurations from embedded t4gfm_config folder into global
...
func PopulateGlobalConfigs() error {
	...
	// we want src/t4gfm_config
	t4gfmConfigDir := filepath.Join(filepath.Dir(filepath.Dir(filepath.Dir(filename))),
		"t4gfm_config")

	configFilePath := filepath.Join(t4gfmConfigDir, "config.toml")
	hotkeyFilePath := filepath.Join(t4gfmConfigDir, "hotkeys.toml")
	themeFilePath := filepath.Join(t4gfmConfigDir, "theme", "monokai.toml")
