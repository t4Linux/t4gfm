...
func TestMain(m *testing.M) {
	...
	// Create testDir
	testDir = filepath.Join(os.TempDir(), "t4gfm_testdir")
	cleanupTestDir()
...
func TestChooserFile(t *testing.T) {
	...
	openWithHotkey := common.Hotkeys.OpenFileWithEditor[0]
	if openWithHotkey == "R" && len(common.Hotkeys.OpenFileWithEditor) > 1 && common.Hotkeys.OpenFileWithEditor[1] != "" {
		openWithHotkey = common.Hotkeys.OpenFileWithEditor[1]
	}
