...
import (
	...
	stringfunction "github.com/t4Linux/t4gfm/src/pkg/string_function"
)
...
const metadataDebounceInterval = 120 * time.Millisecond
...
func (m *model) paneTitle() string {
	loc := m.getFocusedFilePanel().Location
	if loc == "" {
		return "gfm"
	}
	parts := []string{}
	for {
		...
		next := filepath.Dir(loc)
		if next == loc || next == "." || next == string(filepath.Separator) {
			break
		}
		loc = next
	}
...
func (m *model) handleMouseLeftClick(x int, y int) {
	...
	if m.toggleFooter && y >= mainHeightWithBorder {
		footerPanelWidth := m.fullWidth / utils.CntFooterPanels
		if x < footerPanelWidth {
			m.focusPanel = processBarFocus
			m.getFocusedFilePanel().IsFocused = false
			return
		}
		if x < 2*footerPanelWidth {
	...
	if y >= mainHeightWithBorder {
		return
	}

	filePanelColStart := sidebarWidthWithBorder
	for i := range m.fileModel.FilePanels {
		panel := &m.fileModel.FilePanels[i]
		panelColEnd := filePanelColStart + panel.GetWidth()
		if x >= filePanelColStart && x < panelColEnd {
			m.getFocusedFilePanel().IsFocused = false
			m.fileModel.FocusedPanelIndex = i
			m.fileModel.FilePanels[i].IsFocused = true
			m.focusPanel = nonePanelFocus

			clickedIndex := panel.ClickedItemIndex(y)
			if clickedIndex < 0 {
				clickedIndex = panel.ClickedItemIndex(y - 1)
			}
			if clickedIndex >= 0 {
				panel.SetCursorPosition(clickedIndex)
...
func (m *model) updateModelStateAfterMsg() {
	...
	m.systemPanel.SetProcesses(m.processBarModel.SummaryLines(2))
...
func (m *model) getMetadataCmd() tea.Cmd {
	...
	if m.getFocusedFilePanel().EmptyOrInvalid() {
		...
		m.metadataDebouncePath = ""
		m.metadataDebounceAt = time.Time{}
		return nil
	}
	...
	now := time.Now()
	if selectedItem.Location != m.metadataDebouncePath || metadataFocused != m.metadataDebounceMode {
		m.metadataDebouncePath = selectedItem.Location
		m.metadataDebounceMode = metadataFocused
		m.metadataDebounceAt = now
		if m.fileMetaData.IsBlank() {
			m.fileMetaData.SetInfoMsg(icon.InOperation + icon.Space + "Loading metadata...")
		}
		return tea.Tick(metadataDebounceInterval, func(time.Time) tea.Msg {
			return metadataDebounceMsg{}
		})
	}
	if now.Sub(m.metadataDebounceAt) < metadataDebounceInterval {
		return nil
	}
	...
	return func() tea.Msg {
		return NewMetadataMsg(
			metadata.GetMetadata(location, metadataFocused, et), metadataFocused, reqCnt)
	}
}
...
func (m *model) setFooterComponentSize() {
	var width, gitPanelWidth, height int
	height = m.footerHeight + common.BorderPadding
	width = m.fullWidth / utils.CntFooterPanels
	gitPanelWidth = width + m.fullWidth%utils.CntFooterPanels
	m.fileMetaData.SetDimensions(width, height)
	m.systemPanel.SetDimensions(width, height)
	m.processBarModel.SetDimensions(width, height)
	m.gitPanel.SetDimensions(gitPanelWidth, height)
}
...
func (m *model) handleKeyInput(msg tea.KeyMsg) tea.Cmd {
	...
	if key == "ctrl+c" && m.processBarModel.HasRunningProcesses() {
	...
	case m.notifyModel.IsOpen():
		cmd = m.notifyModelOpenKey(key)

	// If renaming a object
	case m.fileModel.Renaming:
		cmd = m.renamingKey(key)
	...
	case m.sortModal.IsOpen():
		m.sortOptionsKey(key)
	// If help menu is open
	case m.helpMenu.IsOpen():
		m.helpMenu.HandleKey(key)

	case slices.Contains(common.Hotkeys.Quit, key):
...
func normalizeKeyInput(msg tea.KeyMsg) string {
	if len(msg.Runes) == 1 {
		r := msg.Runes[0]
		if r >= 'A' && r <= 'Z' {
			return string(r)
...
func (m *model) applyPromptModalAction(action common.ModelAction) tea.Cmd {
	...
	if actionErr != nil {
		m.promptModal.HandlePromptActionResults(false, actionErr.Error())
	} else if successMsg != "" {
		m.promptModal.HandlePromptActionResults(true, successMsg)
	}
	return cmd
}
...
func (m *model) warnModalForQuit() {
	m.notifyModel = notify.New(true, "Confirm to quit t4gfm",
...
func (m *model) updateRenderForOverlay(finalRender string) string {
	...
	if m.rangerPrefix != "" {
		rangerPrefixMenu := m.rangerPrefixMenuRender()
		...
		return stringfunction.PlaceOverlay(overlayX, overlayY, rangerPrefixMenu, finalRender)
	}
