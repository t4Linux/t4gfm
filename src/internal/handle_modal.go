...
func (m *model) confirmOpenWith() tea.Cmd {
	command := strings.TrimSpace(m.typingModal.textInput.Value())
	if command == "" {
		m.typingModal.errorMesssage = "open-with command cannot be empty"
		return nil
	}
	parts := strings.Fields(command)
	if len(parts) == 0 {
		m.typingModal.errorMesssage = "invalid open-with command"
		return nil
	}
	if _, err := exec.LookPath(parts[0]); err != nil {
		m.typingModal.errorMesssage = "program not found: " + parts[0]
		return nil
	}
	targetPath := m.typingModal.targetPath
	if targetPath == "" {
		m.typingModal.errorMesssage = "no selected file to open"
		return nil
	}
